...
1. 등록된 앱 프로세스 트리의 모든 최상위 창에 정상 종료 요청
2. 자식 프로세스를 계속 재열거해 전체 종료 확인 — 창이 없거나 제한시간 초과 시 즉시 프로세스 트리 종료
...
비밀값 패턴 검사는 이번 Push에서 변경·추가된 JSONL과 앱 레지스트리에 수행합니다. 변경된 대형
JSONL은 gzip으로 교체하기 전에 raw 스냅숏을 검사합니다. 스캐너 코드(패턴 포함)가 바뀐 첫 Push는
`.gz`까지 전체 재검사하고 성공한 스캐너 해시를 머신 로컬에 기록하므로, 이후에는 변경분만 읽습니다.
수동 전체 감사가 필요하면 `Push-Sessions.ps1 -Full`을 사용합니다. 다만 모든 민감정보를 보장해
찾아내는 도구는 아닙니다.
...
Finish는 Store 앱 실행 파일을 루트로 삼아 자식까지 계속 재열거하며, 모든 최상위 창에 종료 요청을
보냅니다. 창이 없거나 제한시간을 넘기면 전체 프로세스 트리를 종료하고 실제 PID가 모두 사라진
경우에만 Push를 진행합니다. Finish 자체가 등록 앱의 자식으로 실행 중이면 반쪽짜리 Push를 막기
위해 종료 전에 중단하므로, 독립 PowerShell 창이나 바로가기에서 실행해야 합니다.
