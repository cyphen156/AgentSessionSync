...
$Files = @(foreach ($path in $Paths)
{
    ...
    elseif (Test-Path -LiteralPath $path -PathType Container)
    {
        Get-ChildItem -LiteralPath $path -File -Recurse | Where-Object {
            $_.Name -like '*.jsonl' -or $_.Name -like '*.entry.json' -or
                ($IncludeCompressed -and $_.Name -like '*.jsonl.gz')
        }
    }
})
...
if ($Findings)
{
    ...
    throw '세션 운반물에서 공개하면 안 되는 토큰 후보를 발견했습니다. 실제 값을 출력하지 않고 Push를 중단합니다.'
}
